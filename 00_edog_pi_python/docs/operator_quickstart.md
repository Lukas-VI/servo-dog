...
调参页的滑条会连续拖动，并在约 250 ms 防抖后自动保存到 `config.yaml`。自动视觉闭环运行时会热加载配置，PID、ROI、HSV、岔路参数和任务图坐标会实时生效，不需要再执行 `systemctl restart`。

如果机械磨损导致不易直行，先不要急着加大 P/D：

1. 在架空或低速地面测试中，把 `前进速度` 调到 `0.08` 到 `0.12`。
2. 视觉误差接近 0 但机器人仍持续左/右偏时，先调 `机械偏航 trim`。
   - 持续向左拐：把 `机械偏航 trim` 往负方向调。
   - 持续向右拐：把 `机械偏航 trim` 往正方向调。
3. 如果机器人整体横着漂，再微调 `机械横移 trim`。
4. trim 调到能大致直走后，再调 `转向 P` 和 `转向 D`；P 负责回线力度，D 负责抑制摆头。
5. 每次只改一个参数，小步调整，观察 Web 的 `视觉误差`、`最后串口帧` 和实际姿态。
